...
public class ExampleWork {
    ...
    public static boolean areTheLlamasHappy(boolean ultraBouncy, int trampolines) {
        // are the number of trampolines between 24/42 inclusive?
        boolean isBetween = trampolines > 23 && trampolines < 43;
        boolean above42 = trampolines > 42;
        
        if(isBetween){
            return true;
        } else if(ultraBouncy && above42){
            return true;
        } 
    ...
    public static String howManyWoofs(int numBones) {
        if(numBones < 1){
            ...
            // DON T DO THIS UNLESS YOU ABSOLUITELY HAVE TO
            // TRY TO DO BETTER
            switch(numBones){
                case 1:
                case 3:
