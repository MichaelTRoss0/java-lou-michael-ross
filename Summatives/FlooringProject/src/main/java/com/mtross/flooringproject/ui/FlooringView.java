    ...
    public void displayEditedOrderSummaryBannerAndEditedOrderSummary(Order oldOrder, Order newOrder) {
        ...
        if (!oldOrder.equals(newOrder)) {
            ...
            if (!oldCustomerName.equals(newCustomerName)) {
                io.print("Customer Name: " + oldCustomerName + " -> " + newCustomerName);
            }
            if (!oldState.equals(newState)) {
                io.print("State: " + oldState + " -> " + newState);
            }
            if (oldTaxRate.compareTo(newTaxRate) != 0) {
                io.print("Tax Rate: " + oldTaxRate + "% -> " + newTaxRate + "%");
            }
            if (!oldProductType.equals(newProductType)) {
                io.print("Product Type: " + oldProductType + " -> " + newProductType);
            }
            if (oldArea.compareTo(newArea) != 0) {
                io.print("Area: " + oldArea + " square feet -> " + newArea + " square feet");
            }
            if (oldCostPerSquareFoot.compareTo(newCostPerSquareFoot) != 0) {
                io.print("Cost Per Square Foot: $" + oldCostPerSquareFoot + " -> $" + newCostPerSquareFoot);
            }
            if (oldLaborCostPerSquareFoot.compareTo(newLaborCostPerSquareFoot) != 0) {
                io.print("Labor Cost Per Square Foot: $" + oldLaborCostPerSquareFoot + " -> $" + newLaborCostPerSquareFoot);
            }
            if (oldMaterialCost.compareTo(newMaterialCost) != 0) {
                io.print("Material Cost: $" + oldMaterialCost + " -> $" + newMaterialCost);
            }
            if (oldLaborCost.compareTo(newLaborCost) != 0) {
                io.print("Labor Cost: $" + oldLaborCost + " -> $" + newLaborCost);
            }
            if (oldTax.compareTo(newTax) != 0) {
                io.print("Tax: $" + oldTax + " -> $" + newTax);
            }
            if (oldTotal.compareTo(newTotal) != 0) {
                io.print("Total: $" + oldTotal + " -> $" + newTotal);
            }
